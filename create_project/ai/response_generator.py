...
class ResponseGenerator:
    ...
    def __init__(
        ...
        self._fallback_responses: Dict[PromptType, List[str]] = (
    ...
    async def generate_response(
        ...
        if not self._client.is_available:
            logger.warning(
                "Ollama client not available, using fallback",
        ...
        try:
            ...
            if config.quality_check and not self._validate_response_quality(response):
                logger.warning(
                    "Response failed quality check, using fallback",
                    prompt_type=prompt_type.value,
            ...
            logger.error("Response generation timed out", prompt_type=prompt_type.value)
            raise
    ...
    async def stream_response(
        ...
        if not self._client.is_available:
            logger.warning(
                "Ollama client not available, streaming fallback",
                prompt_type=prompt_type.value,
            )
        ...
        try:
            ...
            if config.quality_check:
                ...
                if not self._validate_response_quality(complete_response):
                    logger.warning(
                        "Streamed response failed quality check",
                        prompt_type=prompt_type.value,
            ...
            logger.error(
                "Response streaming failed", prompt_type=prompt_type.value, error=str(e)
            )
    ...
    async def _stream_fallback(
        self, fallback: str, chunk_size: int = 100
    ) -> AsyncIterator[str]:
    ...
    def _render_prompt(self, prompt_type: PromptType, context: Dict[str, Any]) -> str:
        ...
        try:
            return self._prompt_manager.render_prompt(
                prompt_type, context, validate_required=False
            )
        except Exception as e:
            logger.error(
        ...
        try:
            ...
            if not suitable_models:
                raise ModelNotAvailableError(
                    "No suitable text generation models available", []
                )
    ...
    async def _generate_with_timeout(
        ...
        try:
            ...
            raise ResponseTimeoutError(
                f"Response generation timed out after {config.timeout_seconds}s"
            )
        ...
        try:
            ...
            async for response in stream_task:
                ...
                if (
                    ...
                    stream_task.cancel()
                    raise ResponseTimeoutError(
                        f"Streaming timed out after {config.timeout_seconds}s"
                    )
    ...
    def _get_fallback_response(
        ...
        if prompt_type == PromptType.ERROR_HELP and "error_message" in context:
            ...
            base_response = (
                fallback_list[0]
                if fallback_list
                else "Please check your configuration."
            )
        ...
        return (
            fallback_list[0]
            if fallback_list
            else "Please try checking your project configuration and try again."
        )
