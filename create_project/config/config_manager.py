...
class ConfigManager:
    ...
    def _convert_env_value(self, value: str, config_path: tuple) -> Any:
        ...
        """
        # Boolean values
        if any(path_part in ['debug', 'remember_window_state', 'auto_update', 
                            'enable_cache', 'file_enabled', 'console_enabled'] 
               for path_part in config_path):
            return value.lower() in ('true', '1', 'yes', 'on')
        
        # Integer values
        if any(path_part in ['timeout', 'max_files'] 
               for path_part in config_path) or (
        ...
        if not value.strip() and any(path_part in ['preferred_model'] 
                                    for path_part in config_path):
            return None
    ...
    def _set_nested_dict_value(self, dictionary: Dict[str, Any], 
                              path: tuple, value: Any) -> None:
        """
    ...
    def _merge_config_dicts(self, base: Dict[str, Any], 
