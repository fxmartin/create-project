...
class DirectoryCreator:
    ...
    def __init__(
        self, base_path: Union[str, Path], path_handler: Optional[PathHandler] = None
    ) -> None:
        ...
        self.logger.info("DirectoryCreator initialized", base_path=str(self.base_path))
        ...
        try:
            ...
            if not dry_run and self.created_dirs:
                try:
                    ...
                    self.logger.error(
                        "Rollback failed", rollback_error=str(rollback_error)
                    )
    ...
    def _create_directory(
        self, dir_path: Path, progress_callback: Optional[Callable[[str], None]] = None
    ) -> None:
        ...
        try:
            ...
            if dir_path.exists():
                if dir_path.is_dir():
                    self.logger.debug("Directory already exists", path=str(dir_path))
                    return
                else:
                    raise ProjectGenerationError(
            ...
            if dir_path.is_dir():
                self.logger.debug(
                    "Directory created by another process", path=str(dir_path)
                )
                return
            else:
                raise ProjectGenerationError(
    ...
    def _set_directory_permissions(self, dir_path: Path) -> None:
        ...
        try:
            ...
            self.logger.warning(
                "Could not set directory permissions", path=str(dir_path), error=str(e)
            )
    ...
    def rollback(self) -> None:
        ...
        self.logger.info(
            "Starting directory rollback", directories_to_remove=len(self.created_dirs)
        )
            ...
            try:
                if dir_path.exists() and dir_path.is_dir():
                    ...
                    if not any(dir_path.iterdir()):
                        ...
                        self.logger.warning(
                            "Directory not empty, skipping removal", path=str(dir_path)
                        )
                else:
                    self.logger.debug("Directory no longer exists", path=str(dir_path))
            except OSError as e:
        ...
        if rollback_errors:
            ...
            raise ProjectGenerationError(
                combined_error, details={"rollback_errors": rollback_errors}
            )
        else:
    ...
    def validate_structure(self, structure: Dict[str, Any]) -> None:
        ...
        try:
            ...
            raise ProjectGenerationError(error_msg, original_error=e) from e

    def _validate_structure_recursive(
        self, structure: Dict[str, Any], path_stack: List[str]
    ) -> None:
    ...
    def preview_structure(self, structure: Dict[str, Any]) -> List[str]:
        ...
        def add_preview_recursive(
            struct: Dict[str, Any], indent: int = 0, prefix: str = ""
        ) -> None:
