...
from .exceptions import ProjectGenerationError, TemplateError
...
class FileRenderer:
    ...
    def render_file(
        ...
        try:
            ...
            error_msg = (
                f"Failed to render file '{template_path}' to '{target_path}': {e}"
            )
            self.logger.error(
                error_msg,
                template_path=str(template_path),
    ...
    def _copy_binary_file(
        self, template_path: Path, target_path: Path, executable: bool = False
    ) -> None:
    ...
    def _set_file_permissions(self, file_path: Path, executable: bool = False) -> None:
        ...
        try:
            ...
            self.logger.warning(
                "Could not set file permissions", file_path=str(file_path), error=str(e)
            )
    ...
    def rollback_rendered_files(self) -> None:
        ...
        self.logger.info(
            "Starting file rollback", files_to_remove=len(self.rendered_files)
        )
        ...
        for file_path in reversed(self.rendered_files):
            try:
                if file_path.exists() and file_path.is_file():
                    ...
                    self.logger.debug("File no longer exists", file_path=str(file_path))
            except OSError as e:
        ...
        if rollback_errors:
            ...
            self.logger.error(
                "File rollback completed with errors", errors=rollback_errors
            )
    ...
    def validate_template_file(self, template_path: Union[str, Path]) -> bool:
        ...
        if self._is_binary_file(template_path):
            try:
                ...
                raise TemplateError(
                    f"Cannot read binary template file '{template_path}': {e}"
                )
    ...
    def preview_rendered_content(
        ...
        try:
            ...
            if len(lines) > max_lines:
                ...
                lines.append(
                    f"... ({len(rendered.split(chr(10))) - max_lines} more lines)"
                )
