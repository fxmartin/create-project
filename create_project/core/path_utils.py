...
class PathHandler:
    ...
    def normalize_path(self, path: Union[str, Path]) -> Path:
        ...
        try:
            ...
            self.logger.debug(
                "Path normalized", original=str(path), normalized=str(normalized)
            )
    ...
    def safe_join(self, base_path: Union[str, Path], *paths: Union[str, Path]) -> Path:
        ...
        try:
            ...
            for path_component in paths:
                ...
                if component.is_absolute() or self._is_windows_absolute_path(
                    component_str
                ):
                    error_msg = (
                        f"Absolute path components not allowed: '{path_component}'"
                    )
                    self.logger.error(
                        error_msg, attempted_component=str(path_component)
                    )
                    raise SecurityError(error_msg)
    ...
    def validate_filename(self, filename: str) -> None:
        ...
        if invalid_chars:
            raise PathError(
                f"Filename contains invalid characters: {sorted(invalid_chars)}"
            )
