...
class BackgroundOperation:
    ...
    def start(self, executor: ThreadPoolExecutor) -> None:
        ...
        with self._lock:
            ...
            self.logger.info(
                "Background operation started", operation_id=self.operation_id
            )
        ...
        with self._lock:
            ...
            if self.future:
                ...
                if cancelled or self.cancellation_event.is_set():
                    ...
                    self.logger.info(
                        "Background operation cancelled", operation_id=self.operation_id
                    )
    ...
    def get_result(self, timeout: Optional[float] = None) -> OperationResult:
        ...
        if not self.future:
            raise ThreadingError(
                "Operation not started", details={"operation_id": self.operation_id}
            )
...
class ThreadingModel:
    ...
    def __init__(self, max_workers: Optional[int] = None) -> None:
        ...
        self.logger.info("ThreadingModel initialized", max_workers=max_workers)
    ...
    def start_project_generation(
        ...
        operation = BackgroundOperation(
            ...
            },  # We'll handle progress internally
    ...
    def cancel_operation(self, operation_id: str) -> bool:
        ...
        if not operation:
            self.logger.warning(
                "Attempted to cancel non-existent operation", operation_id=operation_id
            )
            return False
    ...
    def list_active_operations(self) -> Dict[str, OperationStatus]:
        ...
        with self._operations_lock:
            return {
                op_id: operation.status for op_id, operation in self.operations.items()
            }
    ...
    def cleanup_completed_operations(self) -> int:
        ...
        if removed_count > 0:
            self.logger.info(
                "Cleaned up completed operations", removed_count=removed_count
            )
