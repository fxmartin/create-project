...
from .exceptions import VirtualEnvError
...
class VenvManager:
    ...
    def __init__(self) -> None:
        ...
        self.logger.info(
            ...
            available_tools=[
                tool.value for tool, path in self.available_tools.items() if path
            ],
    ...
    def create_venv(
        ...
        try:
            ...
            result = self._create_with_tool(
                self.preferred_tool, project_path, venv_name, python_version
            )
            ...
            for tool in self.TOOL_PRIORITY:
                ...
                try:
                    result = self._create_with_tool(
                        tool, project_path, venv_name, python_version
                    )
            ...
            if venv_path.exists():
                try:
                    ...
                    self.logger.debug(
                        "Cleaned up failed venv creation", venv_path=str(venv_path)
                    )
    ...
    def get_activation_instructions(
        self, project_path: Path, venv_name: str = ".venv"
    ) -> Dict[str, str]:
        ...
        if not venv_path.exists():
            return {"error": f"Virtual environment not found: {venv_path}"}
    ...
    def _detect_available_tools(self) -> None:
        ...
        if uv_path:
            try:
                ...
                if result.returncode == 0:
                    ...
                    self.logger.debug(
                        "uv detected", path=uv_path, version=result.stdout.strip()
                    )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.debug("venv detected", python_executable=sys.executable)
        except (subprocess.TimeoutExpired, Exception) as e:
    ...
    def _create_with_tool(
        ...
        try:
            if tool == VenvTool.UV:
                ...
                return self._create_with_virtualenv(
                    tool_path, venv_path, python_version
                )
            ...
            return {"success": False, "error": str(e), "tool": tool.value}

    def _create_with_uv(
        self, uv_path: str, venv_path: Path, python_version: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        result = subprocess.run(
            command, capture_output=True, text=True, timeout=120, cwd=venv_path.parent
        )
        ...
        return {
            ...
            "tool": VenvTool.UV.value,
            "python_version": python_version,
            "activation_instructions": self.get_activation_instructions(
                venv_path.parent, venv_path.name
            ),
        }
    ...
    def _create_with_venv(
        self, python_path: str, venv_path: Path, python_version: Optional[str] = None
    ) -> Dict[str, Any]:
