...
class TemplateCompatibility(BaseModel):
    ...
    max_python_version: Optional[str] = Field(
        None,
        description="Maximum Python version supported"
    )
...
class TemplateUsageStats(BaseModel):
    ...
    downloads: int = Field(
        default=0,
        description="Number of times template was downloaded"
    )

    generations: int = Field(
        default=0,
        description="Number of times template was used to generate projects"
    )
...
class Template(BaseTemplate):
    ...
    variables: List[TemplateVariable] = Field(
        default_factory=list,
        description="Template variables for user input"
    )
    ...
    action_groups: List[ActionGroup] = Field(
        default_factory=list,
        description="Grouped actions for organization"
    )
        ...
        for file in all_files:
            ...
            for var in file_vars:
                if var not in defined_vars:
                    errors.append(f"Undefined variable '{var}' used in file name: {file.name}")

            # Check content if it's inline
            if file.content:
                content_vars = re.findall(r"\{\{\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*\}\}", file.content)
                for var in content_vars:
                    if var not in defined_vars:
                        errors.append(f"Undefined variable '{var}' used in file content: {file.name}")

        # Check variables in directory names
        all_dirs = self.structure.get_all_directories()
        for directory in all_dirs:
            dir_vars = re.findall(r"\{\{\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*\}\}", directory.name)
            for var in dir_vars:
                if var not in defined_vars:
                    errors.append(f"Undefined variable '{var}' used in directory name: {directory.name}")
    ...
    def _validate_template_file_references(self, errors: List[str]):
        ...
        for file in all_files:
            if file.template_file and file.template_file not in available_templates:
                errors.append(f"Template file '{file.template_file}' not found (referenced by {file.name})")
