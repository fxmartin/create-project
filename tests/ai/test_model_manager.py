...
from create_project.ai.exceptions import ModelNotAvailableError, AIError
...
class TestModelFetching:
    ...
    def test_caching_behavior(self):
        ...
        # Mock successful API response
        response_data = {"models": [{"name": "test:latest", "size": "1000", "digest": "sha256:test", "modified_at": "2024-01-01T12:00:00Z"}]}
        mock_response = OllamaResponse(success=True, status_code=200, data=response_data)
        self.mock_client.get_models.return_value = mock_response
    ...
    def test_cache_expiry(self):
        ...
        self.manager._cache = []
        self.manager._cache_timestamp = datetime.now() - timedelta(minutes=15)  # Expired
...
class TestModelQueries:
    ...
    def test_get_models_by_capability(self):
        ...
        text_models = self.manager.get_models_by_capability(ModelCapability.TEXT_GENERATION)
        assert len(text_models) == 2  # llama2 and codellama
        
        # Test CODE_GENERATION capability
        code_models = self.manager.get_models_by_capability(ModelCapability.CODE_GENERATION)
    ...
    def test_concurrent_model_fetching(self):
        ...
        def slow_get_models():
            ...
            return OllamaResponse(
                success=True,
                status_code=200,
                data={"models": [{"name": "test:latest", "size": "1000", "digest": "sha256:test", "modified_at": "2024-01-01T12:00:00Z"}]}
            )
