...
class TestOllamaResponse:
    ...
    def test_successful_response(self):
        ...
        response = OllamaResponse(
            success=True, status_code=200, data={"models": []}, response_time=0.5
        )
...
class TestOllamaClientInitialization:
    ...
    def test_custom_initialization(self):
        ...
        client = OllamaClient(
            base_url="http://custom:8080", timeout=60.0, retry_config=retry_config
        )
...
class TestAPIHelperMethods:
    ...
    def test_generate_completion(self, mock_request):
        ...
        response = client.generate_completion(
            model="llama2", prompt="Hello", temperature=0.8
        )
        ...
        mock_request.assert_called_once_with(
            RequestMethod.POST, "generate", data=expected_data
        )
    ...
    def test_chat_completion(self, mock_request):
        ...
        expected_data = {"model": "llama2", "messages": messages, "stream": False}

        assert response is mock_response
        mock_request.assert_called_once_with(
            RequestMethod.POST, "chat", data=expected_data
        )
