...
class TestTemplateEngineIntegration:
    ...
    def test_variable_resolution_with_complex_conditions(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # cli_tool should be None since it's not shown for web projects
            assert resolved_vars.get("cli_tool") is None
            ...
            # web_framework may still have its default value even if hidden
            # This is current engine behavior - variables with defaults are included
            ...
            # Variables with defaults will still have their defaults
            # cli_tool has no default so it should be None
            assert resolved_vars.get("cli_tool") is None
