...
class TestAIErrorHandling:
    ...
    async def test_git_initialization_error_recovery(
        ...
        def mock_run(*args, **kwargs):
            if "git" in args[0]:
                raise subprocess.CalledProcessError(128, args[0], "fatal: not a git repository")
    ...
    async def test_error_context_sanitization(
        ...
        sensitive_path = Path(f"/home/johndoe/secret_project")
        error = PathError(
            f"Cannot write to {sensitive_path}"
        )
...
class TestAIErrorRecoveryWorkflows:
    ...
    @pytest.mark.asyncio
    @pytest.mark.slow
    async def test_full_error_recovery_workflow(
        self,
        tmp_path,
        mocker: MockerFixture
    ):
    ...
    @pytest.mark.asyncio
    async def test_multi_stage_error_recovery(
        self,
        tmp_path,
        mocker: MockerFixture
    ):
        ...
        def mock_subprocess_run(*args, **kwargs):
            ...
            elif error_stage == 1 and "python" in str(cmd) and "-m" in cmd and "venv" in cmd:
                error_stage = 2
        ...
        ai_responses = [
            {
                "message": {
                    "content": "Git not found. Install with: brew install git"
                },
                "done": True
            },
            {
