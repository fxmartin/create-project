...
from create_project.ai.exceptions import OllamaNotFoundError, AIError
...
class TestAIProjectGeneration:
    ...
    def _load_template(self, template_loader: TemplateLoader, template_name: str) -> Template:
    ...
    async def test_successful_project_generation_with_ai(
        ...
        print(f"Target path contents: {list(target_path.iterdir()) if target_path.exists() else 'Directory does not exist'}")
    ...
    async def test_project_generation_with_template_validation_errors(
        ...
        template = self._load_template(template_loader, "CLI Application (Single Package)")
    ...
    async def test_project_generation_with_custom_config(
        ...
        mock_client = MockOllamaClient(
            available_models=[MockModelData.CODELLAMA_7B]
        )
        mocker.patch(
            "create_project.ai.ollama_client.OllamaClient.__init__",
            return_value=None
        )
        mocker.patch(
            "create_project.ai.ollama_client.OllamaClient.__new__",
...
class TestAIProjectGenerationEdgeCases:
    ...
    async def test_project_generation_with_corrupted_cache(
        ...
        mocker.patch("platformdirs.user_cache_dir", return_value=str(cache_dir.parent.parent))
    ...
    async def test_project_generation_with_unicode_content(
        ...
        assert result.success
        readme_content = (tmp_path / "unicode_test" / "README.md").read_text(encoding="utf-8")
