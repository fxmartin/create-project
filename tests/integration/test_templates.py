...
from create_project.templates.schema import (
    Template, TemplateMetadata, TemplateConfiguration,
    TemplateVariable, ProjectStructure, DirectoryItem, FileItem,
    ValidationRule, ConditionalExpression, VariableType,
    TemplateAction, ActionType, TemplateHooks
)


def create_mock_one_off_script_template() -> Template:
    """Create a mock one-off script template."""
    from create_project.templates.schema.template import TemplateCompatibility, TemplateFiles
...
def get_mock_template(name: str) -> Template:
    ...
    if name not in MOCK_TEMPLATES:
        raise ValueError(f"Mock template '{name}' not found. Available: {list(MOCK_TEMPLATES.keys())}")
