...
class TestBuiltinTemplates:
    ...
    def builtin_templates_dir(self) -> Path:
        ...
        return Path(__file__).parent.parent.parent / "create_project" / "templates" / "builtin"
    ...
    def test_all_templates_exist(self, template_files: Dict[str, Path]):
        ...
        assert actual_templates == expected_templates, f"Missing templates: {expected_templates - actual_templates}"
    ...
    def test_templates_have_required_fields(self, template_files: Dict[str, Path]):
        ...
        for template_name, template_path in template_files.items():
            ...
            assert not missing_fields, f"Template {template_name} missing fields: {missing_fields}"
    ...
    def test_templates_pass_comprehensive_validation(self, template_files: Dict[str, Path], validator: TemplateValidator):
    ...
    def test_template_metadata_completeness(self, template_files: Dict[str, Path]):
        ...
        for template_name, template_path in template_files.items():
            ...
            required_metadata = {"name", "description", "version", "category", "author", "template_id"}
            missing_metadata = required_metadata - set(metadata.keys())
            assert not missing_metadata, f"Template {template_name} missing metadata: {missing_metadata}"
            
            # Check that template_id follows convention
            expected_id = f"builtin_{template_name}"
            assert metadata["template_id"] == expected_id, f"Template {template_name} has incorrect ID"
    
    def test_template_variables_have_required_fields(self, template_files: Dict[str, Path]):
        ...
        for template_name, template_path in template_files.items():
            ...
            for i, variable in enumerate(variables):
                ...
                assert not missing_fields, f"Template {template_name} variable {i} missing fields: {missing_fields}"
    ...
    def test_template_structure_validity(self, template_files: Dict[str, Path]):
        ...
        for template_name, template_path in template_files.items():
            ...
            assert "name" in structure, f"Template {template_name} structure missing name"
            ...
            assert has_files or has_dirs, f"Template {template_name} structure has no files or directories"
    ...
    def test_template_compatibility_requirements(self, template_files: Dict[str, Path]):
        ...
        for template_name, template_path in template_files.items():
            ...
            assert "min_python_version" in compatibility, f"Template {template_name} missing min_python_version"
            python_version = compatibility["min_python_version"]
            
            # Validate Python version format (should be like "3.9.6")
            version_parts = python_version.split(".")
            assert len(version_parts) == 3, f"Template {template_name} invalid Python version format"
            
            # Check supported OS
            assert "supported_os" in compatibility, f"Template {template_name} missing supported_os"
            supported_os = compatibility["supported_os"]
            assert isinstance(supported_os, list), f"Template {template_name} supported_os must be a list"
    ...
    def test_individual_template_loads(self, template_name: str, builtin_templates_dir: Path):
    ...
    def test_template_variable_defaults(self, template_files: Dict[str, Path]):
        ...
        for template_name, template_path in template_files.items():
            ...
            assert not missing_common, f"Template {template_name} missing common variables: {missing_common}"
    ...
    def test_template_hooks_are_valid(self, template_files: Dict[str, Path]):
        ...
        for template_name, template_path in template_files.items():
            ...
            assert "post_generation" in hooks, f"Template {template_name} missing post_generation hooks"
            
            # Validate each hook
            for hook in hooks.get("post_generation", []):
                assert "type" in hook, f"Template {template_name} hook missing type"
                assert "command" in hook, f"Template {template_name} hook missing command"
                assert "description" in hook, f"Template {template_name} hook missing description"
    
    def test_template_files_references_exist(self, template_files: Dict[str, Path], builtin_templates_dir: Path):
        ...
        for template_name, template_path in template_files.items():
            ...
            template_file_refs = template_data.get("template_files", {}).get("files", [])
            
            for ref in template_file_refs:
                ref_name = ref["name"]
                ref_path = template_files_dir / ref_name
                assert ref_path.exists(), f"Template {template_name} references non-existent file: {ref_name}"
    ...
    def test_license_choices_are_valid(self, template_files: Dict[str, Path]):
        ...
        for template_name, template_path in template_files.items():
            ...
            if license_var and "choices" in license_var:
                ...
                assert not invalid_licenses, f"Template {template_name} has invalid license choices: {invalid_licenses}"
            ...
            for variable in template.variables:
                if variable.name in sample_vars:
                    ...
                    if hasattr(variable, 'validate_value'):
                        ...
                        assert len(errors) == 0, f"Sample variable {variable.name} validation failed: {errors}"