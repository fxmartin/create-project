...
class TestLogRotation(unittest.TestCase):
    ...
    def test_log_rotation_by_size(self):
        ...
        rotating_handlers = [
            h
            for h in python_logger.handlers
            if isinstance(h, logging.handlers.RotatingFileHandler)
        ]
        self.assertTrue(
            len(rotating_handlers) > 0, "Should have rotating file handlers"
        )
    ...
    def test_error_log_rotation_separate(self):
        ...
        self.assertTrue(
            len(error_handlers) > 0, "Should have error log rotating handler"
        )

        for handler in error_handlers:
            self.assertEqual(
                handler.level, logging.ERROR, "Error handler should be ERROR level"
            )
    ...
    def test_log_file_permissions(self):
        ...
        self.assertTrue(
            stat.st_mode & stat_module.S_IRUSR, "Owner should have read permission"
        )
        self.assertTrue(
            stat.st_mode & stat_module.S_IWUSR, "Owner should have write permission"
        )
...
class TestCrossComponentLogging(unittest.TestCase):
    ...
    def test_performance_under_load(self):
        ...
        self.assertLess(
            duration, 5.0, "Logging 1000 messages should complete within 5 seconds"
        )
        ...
        with open(app_log) as f:
            ...
            self.assertGreater(
                len(lines),
                num_messages // 2,
                "Should have substantial number of log entries",
            )
