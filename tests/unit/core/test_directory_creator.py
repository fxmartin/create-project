...
from create_project.core.exceptions import ProjectGenerationError, PathError
...
class TestDirectoryCreator:
            ...
            with patch('pathlib.Path.is_dir', return_value=True):
                with patch('pathlib.Path.iterdir', return_value=[]):
                    with patch('pathlib.Path.rmdir', mock_rmdir):
                        ...
                        assert "rollback completed with errors" in str(exc_info.value).lower()
    ...
    def test_error_handling_with_rollback(self, creator):
        ...
        with patch.object(creator, '_create_directory', side_effect=failing_create_directory):
    ...
    def test_safe_path_joining(self, creator):
        ...
        with pytest.raises(Exception):  # Could be SecurityError or ProjectGenerationError
...
class TestDirectoryCreatorEdgeCases:
    ...
    def test_concurrent_creation_simulation(self, creator):
        ...
        with patch.object(creator, '_create_directory', side_effect=mock_create_with_race):
