...
class TestFileRenderer:
    ...
    def test_render_file_with_progress_callback(self, renderer, temp_dir):
        ...
        renderer.render_file(
            template_file, target_file, {"value": "test"}, 
    ...
    def test_render_files_from_structure_nested(self, renderer, temp_dir):
        ...
        (template_dir / "src" / "utils" / "helper.py.j2").write_text("# Helper: {{ project }}")
        ...
        assert "Helper: MyProject" in (target_dir / "src" / "utils" / "helper.py").read_text()
    ...
    def test_detect_encoding_fallback(self, renderer, temp_dir):
        ...
        with patch('chardet.detect', return_value={'encoding': None, 'confidence': 0.1}):
    ...
    def test_preview_rendered_content_text(self, renderer, temp_dir):
        ...
        preview = renderer.preview_rendered_content(template_file, variables, max_lines=5)
    ...
    def test_preview_rendered_content_truncated(self, renderer, temp_dir):
        ...
        variables = {"value": "test"}
        preview = renderer.preview_rendered_content(template_file, variables, max_lines=10)
