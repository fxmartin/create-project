...
class TestPathHandler:
    ...
    def test_normalize_path_unicode(self, handler, temp_dir):
        ...
        assert isinstance(normalized, Path)
        assert "tëst" in str(normalized) or "te\u0302st" in str(normalized)  # NFC normalization
    ...
    def test_error_handling_preserves_original_error(self, handler):
        ...
        with patch.object(handler, 'normalize_path', side_effect=ValueError("Test error")):
            with pytest.raises(PathError) as exc_info:
    ...
    @pytest.mark.parametrize("os_name,expected_case_sensitive", [
        ("Windows", False),
        ("Linux", True),
        ("Darwin", True),
        ("FreeBSD", True),
